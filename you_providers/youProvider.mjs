                    ...
                    if (COOKIE_PERSISTENCE_MODE) {
                        try {
                            ...
                            if (existingSessionCookie && existingSessionCookie.email) {
                                ...
                                console.log(`[${currentUsername}] 使用浏览器缓存Cookie信息 (${browserEmail})`);
                                // 合并Cookie到session
                                ...
                                console.log(`[${currentUsername}] 浏览器中无有效Cookie，使用配置文件中Cookie`);
                                await page.setCookie(...getSessionCookie(extractCookieFields(session)));
                                await page.reload({ waitUntil: 'domcontentloaded' });
                            }
                        } catch (error) {
                            console.warn(`[${currentUsername}] Failed to check browser Cookie:`, error);
                            await page.setCookie(...getSessionCookie(extractCookieFields(session)));
                            await page.goto("https://you.com/?chatMode=custom", {
                                timeout,
                                waitUntil: 'domcontentloaded'
                    ...
                    try {
                        ...
                        if (typeof content === 'string') {
                            ...
                            if (content.includes("User account or domain blocked for abuse")) {
                                console.error(`[${currentUsername}] 账号被封禁: ${content}`);
                                await markAccountAsInvalid(currentUsername, this.config, "账号或域名因滥用被封禁");
                                session.valid = false;
                                return;
                            } else if (contentLower.includes("blocked")) {
                                console.error(`[${currentUsername}] 账号被封禁: ${content}`);
                                await markAccountAsInvalid(currentUsername, this.config, "账号被封禁");
                                session.valid = false;
                                return;
                            }
                        }
                        ...
                        if (session.isTeamAccount) {
                            ...
                            // 标记为失效
                            await markAccountAsInvalid(currentUsername, this.config, "无有效订阅");
                        }
                    } catch (error) {
                        ...
                        if (error instanceof SyntaxError) {
                            console.error(`[${currentUsername}] JSON解析错误: ${error.message}`);
                            console.error(`[${currentUsername}] 原始响应: ${content}`);
                        } else {
                            console.error(error);
                        }
                        ...
                        // 标记为失效
                        await markAccountAsInvalid(currentUsername, this.config, "API请求失败");
                    }
                } catch (errorVisit) {
