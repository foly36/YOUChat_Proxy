...
class ModelManager {
    constructor() {
        ...
        this.defaultBuildHash = "b60b7830314aab3b3c49b3e34b7cdbe2f48c2751"; // 默认哈希值
    ...
    detectRegion() {
        ...
        try {
            ...
            if (timezone === 'Asia/Shanghai' || timezone === 'Asia/Chongqing' || timezone === 'Asia/Harbin' ||
                timezone === 'Asia/Urumqi' || timezone === 'Asia/Hong_Kong') {
                this.isChinaRegion = true;
                this.regionDetected = true;
                return;
            }
    ...
    async fetchModelsFromApi() {
        ...
        if (this.useCloudflareWorker) {
            ...
            if (workerModels && workerModels.length > 0) {
                ...
                await this.saveModelsToCache(workerModels);
                return workerModels;
            }
        }
    ...
    async refreshModels() {
        try {
            ...
            if (!this.areModelListsEqual(newModels, referenceModels)) {
                console.log('Model list updated');
                await this.saveModelsToCache(newModels);
            } else {
                console.log('Model list unchanged');
            }
    ...
    startAutoRefresh(interval = 60 * 60 * 1000) { // 1小时
        this.refreshModels();
        setInterval(() => this.refreshModels(), interval);
    }
